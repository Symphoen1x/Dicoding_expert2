...
class MyCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs = {}):
        thres = .1
        if logs.get('mae') is not None and logs.get('mae') < thres:
            print(f"\nThe desired training MAE is {logs.get('mae')}, that is less than {thres}. but, the desired training MSE is {logs.get('mse')}\n\n")
...
def solution_A5():
    ...
    model.fit(train_set, batch_size=256, epochs=120, callbacks=[callbacks])
    return model
